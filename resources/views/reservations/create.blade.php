...
@section('content')
    <div class="max-w-3xl mx-auto py-6">
        ...
        <form action="{{ route('reservations.store') }}" method="POST" class="bg-white shadow-lg rounded-lg p-6">
            ...
            <!-- Amount of Adults -->
            <div class="mb-4">
                <label for="adult_count" class="block text-gray-700">Amount of Adults</label>
            ...
            <!-- Amount of Kids -->
            <div class="mb-4">
                <label for="child_count" class="block text-gray-700">Amount of Kids</label>
            ...
            <div class="mb-4">
                ...
                <select name="package_option" id="package_option" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    <option value="snackpacketbasis">Snack Packet Basis</option>
                    <option value="snackpakketluxe">Snack Packet Luxe</option>
            ...
            <!-- Lane Number -->
            <div class="mb-4">
                <label for="lane_id" class="block text-gray-700">Lane Number</label>
                <select name="lane_id" id="lane_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    @foreach ($lanes as $lane)
                        <option value="{{ $lane->id }}">{{ $lane->number }} (Has Fence: {{ $lane->has_fence ? 'Yes' : 'No' }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="opening_time_id" id="opening_time_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    @foreach ($openingTimes as $openingTime)
                        <option value="{{ $openingTime->id }}">{{ $openingTime->day_name }} - {{ $openingTime->start_time }} to {{ $openingTime->end_time }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-blue-500 text-white rounded-md py-2 px-6 mt-4 hover:bg-blue-600">Create Reservation</button>
            </div>
        </form>
    </div>
@endsection
